...
void.tx = glcm::glcm(gray.peat,n_grey = 7,window = c(9,9), 
                     statistics = c("second_moment","homogeneity","contrast","entropy"),
...
rot.df<-rot.df%>%
  ...
  mutate(exag.rootpx = rotation.rootpx**1.33)
...
ggplot(data= rot.df, 
  ...
  geom_bar(fill="indianred",
    ...
    geom_text(data=rot.df,
            aes(y=max(exag.rootpx)-max(exag.rootpx)*0.85,label=rotation),hjust = 0.5, size = 3,color = "white")+
  theme_minimal()+
  ylab("")+xlab("")+
...
coord_polar(start= (max(rot.df$rotation)/2)-(ryth.test$phase - ryth.test$peakTime) )
