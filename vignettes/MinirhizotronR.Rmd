...
seg_Oulanka2023_Session03_T067 = blur.correction(seg_Oulanka2023_Session03_T067,0.1)
...
rootzone = zone.fun(rootpic = gray.roots, binned.map = bm, indexD = 20, nn = 2 )
...
rsm = RootScapeMetrics(rootzone,indexD = 20, metrics = c( "lsm_c_ca","lsm_l_ent","lsm_c_pd",
                                      "lsm_c_enn_mn")) 
print(rsm) 
...
buffer.sum = terra::zonal( raster::brick(buff.im),bm, "sum" )
...
peat.color = raster::crop(rgb_Oulanka2023_Session03_T067,gray.roots)
...
root.color = raster::crop(rgb_Oulanka2023_Session03_T067,gray.roots)
...
## extent don't match
bm2 = raster::raster(raster::as.array(bm)[,1:3000,])
raster::extent(bm2) = raster::extent(void.tx) 
tx.mean = raster::zonal(void.tx[[1]],bm2,"mean")
...
for (k in 1:splits) {
rotation.z = zone.rotation.fun(rootpic = root.seg, mm= c(4000,9000), kk = splits, k = c(k-1,k))  
rot.df[k,2] = px.sum(root.zone = rotation.z)
}
rot.df$sc.rotationpx = scale(rot.df$rotation.rootpx)
ggplot(data= rot.df, aes(rotation,sc.rotationpx)) + geom_point() +  theme_bw() + xlab("rotation cm") + ylab("root px") + geom_smooth(method="lm", formula = y ~ cos(x*pi/8),method.args = list(start=c(b=8)),aes(color = "cosine fit")) + geom_smooth(se=F,aes(color="local smooth"),span = 0.5)
