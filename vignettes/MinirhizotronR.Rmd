...
```{r Input Example Showcase, fig.height=4.9, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE,eval=FALSE,cache=FALSE}
# p1 <- ggdraw() + draw_image(paste0(getwd(),"/demo_images/Oulanka2023_Session03_T067.jpg"), scale = 0.6)
# p2 <- ggdraw() + draw_image(paste0(getwd(),"/demo_images/Oulanka2023_Session03_T067_segmented.jpg"), scale = 0.6)
# p3 <- ggdraw() + draw_image(paste0(getwd(),"/demo_images/Oulanka2023_Session03_T067_skeleton.jpg"),scale = 0.6)
...
```{r Calibrating, echo=TRUE, message=FALSE, warning=FALSE, cache=FALSE, eval=TRUE}
...
```{r Create a depthmap, echo=FALSE, fig.height=4, fig.width=1.5, warning=FALSE, eval=TRUE}
...
DepthMap = MinirhizotronR::create.depthmap(seg_Oulanka2023_Session03_T067, mask = mask,start.soil = s0$soil0, center.offset = center.offset )
...
```{r Depth Stats, fig.height=3, fig.width=4.2, , warning=FALSE, eval=TRUE}
...
```{r standardize to available space, eval=FALSE,cache=FALSE}
...
```{r rhizosphere tangent, eval=FALSE}
## set hard limits
root.seg = gray.roots 
...
```{r Characterize the soil, eval=FALSE}
...
## extent don't match
bm2 = terra::raster(terra::as.array(bm)[,1:3000,])
terra::ext(bm2) = terra::ext(void.tx) 
tx.mean = terra::zonal(void.tx[[1]],bm2,"mean")
...
```{r Asess Rotational Bias, eval=FALSE,cache=FALSE}
...
coord_polar(start=(splits - ryth.test$phase )*1.3)
...
turn3 = Turnover.DPC(TurnoverDPC_data, im.return = T)
# adapt depth map

bm.turn = RotCensor(bm)
#bm.turn2 = DepthLimiter(bm.turn,top.cut = F)
#bm.turn2 =raster::raster(bm.turn2)
# terra::ext(bm.turn) = terra::ext(turn3$production)
# prd = raster::zonal(turn3$production,bm.turn,"sum")
# dec = raster::zonal(turn3$decay,bm.turn,"sum")
